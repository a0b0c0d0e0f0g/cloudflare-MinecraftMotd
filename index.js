...
async function generateSvgString(serverIP, env) {
    ...
    const svg = `<svg width="${cardWidth}" height="${h}" viewBox="0 0 ${cardWidth} ${h}" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="35" y="${headerHeight + 88}" font-family="Arial" font-size="11" fill="#94e2d5" font-weight="bold" style="letter-spacing:1.5px" class="sh">ONLINE PLAYERS</text>
        <foreignObject x="35" y="${headerHeight + 98}" width="390" height="${playerAreaHeight}"><div xmlns="http://www.w3.org/1999/xhtml" class="pc" style="font-size:12px;line-height:1.6">${pListHtml}</div></foreignObject>
...
.input-wrapper {
    ...
    overflow: hidden;
    margin-bottom: 18px;
    box-shadow: 0 10px 24px rgba(10,18,30,0.35);
...
textarea:focus{
    border:1px solid rgba(139,211,255,0.6);
    box-shadow: 0 0 0 3px rgba(139,211,255,0.2), inset 0 3px 10px rgba(0,0,0,0.3);
    background:rgba(9,20,34,0.45);
}
.input-wrapper:focus-within{
    box-shadow: 0 12px 28px rgba(10,18,30,0.45);
}
