...
<head>
    ...
    <style>
        ...
        .logo { width: 70px; height: 70px; margin-bottom: 15px; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); }
        h2 { margin: 0; color: #ffffff; font-size: 22px; text-shadow: 0 2px 4px rgba(0,0,0,0.3); }
        p.desc { color: rgba(255, 255, 255, 0.9); font-size: 13px; margin: 12px 0 25px; text-shadow: 0 1px 2px rgba(0,0,0,0.3); }
        input { 
            width: 100%; 
            padding: 14px; 
            margin-bottom: 15px; 
        ...
        button:hover { background: #b4befe; transform: scale(1.02); }
...
<body>
    <div class="box">
        ...
        <input type="text" id="ip" placeholder="例如: play.hypixel.net" onkeypress="if(event.keyCode==13)gen()">
        <button onclick="gen()">生成预览卡片</button>
        <div id="res"></div>
        <div class="footer">© 2024 MC Card Tool • Cloudflare Workers</div>
    </div>
    <script>
        function gen() {
            ...
            resDiv.innerHTML = '<p style="color:white; font-size:12px;">获取中...</p>';
