...
async function handleImageRequest(serverIP) {
  ...
  try {
    const res = await fetch(apiUrl, { headers: { "User-Agent": "MC-Status-Worker/1.4" }, cf: { cacheTtl: 60 } });
    ...
    const cleanMotd = (isOnline ? data.motd : "Server is currently offline.").replace(/§[0-9a-fk-or]/gi, "").replace(/\n/g, " ").substring(0, 45);
    ...
    const svg = `<svg width="450" height="150" viewBox="0 0 450 150" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style="stop-color:#1e1e2e;stop-opacity:1" /><stop offset="100%" style="stop-color:#181825;stop-opacity:1" /></linearGradient></defs><rect width="450" height="150" rx="20" fill="url(#g)" stroke="#313244" stroke-width="2"/><image href="${iconData}" x="25" y="30" width="64" height="64" /><text x="105" y="45" font-family="Arial,sans-serif" font-size="20" fill="#cdd6f4" font-weight="bold">${serverIP}</text><text x="105" y="65" font-family="Arial,sans-serif" font-size="12" fill="#fab387">Version: ${version}</text><text x="105" y="88" font-family="Arial,sans-serif" font-size="14" fill="#bac2de">${cleanMotd}</text><rect x="105" y="105" width="100" height="22" rx="11" fill="${isOnline ? '#a6e3a1' : '#f38ba8'}" fill-opacity="0.15"/><circle cx="118" cy="116" r="4" fill="${isOnline ? '#a6e3a1' : '#f38ba8'}"/><text x="130" y="120" font-family="Arial,sans-serif" font-size="12" font-weight="bold" fill="${isOnline ? '#a6e3a1' : '#f38ba8'}">${isOnline ? playersNow + ' / ' + playersMax : 'OFFLINE'}</text><text x="215" y="120" font-family="Arial,sans-serif" font-size="11" fill="#94e2d5" font-style="italic">Players: ${playerListText}</text></svg>`;
...
<head>
    ...
    <style>
        ...
        body { background: var(--bg); color: var(--text); font-family: system-ui; transition: 0.3s; display: flex; flex-direction: column; align-items: center; min-height: 100vh; margin: 0; padding-top: 40px; }
        .container { background: var(--card); padding: 2rem; border-radius: 20px; width: 90%; max-width: 480px; text-align: center; }
        ...
        input { width: 100%; box-sizing: border-box; padding: 14px; margin: 15px 0; border: none; border-radius: 12px; background: var(--input); color: var(--text); }
        button { width: 100%; padding: 14px; border: none; border-radius: 12px; background: var(--btn); color: var(--btn-text); font-weight: bold; cursor: pointer; }
        #preview { margin-top: 30px; }
        #preview img { max-width: 100%; border-radius: 10px; }
        .code-box { background: #181825; color: #a6e3a1; padding: 15px; margin-top: 20px; border-radius: 10px; font-size: 12px; display: none; word-break: break-all; }
    </style>
</head>
<body data-theme="dark">
    ...
    <div class="container">
        <h2>MC 状态卡片 v2.0</h2>
        <input type="text" id="ip" placeholder="输入服务器 IP">
